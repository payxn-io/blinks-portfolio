...
export default function Swap() {
  ...
  const generateSwapLink = async () => {
    const tokenPair = 'SOL-USDT';
    const localhost = 'http://localhost:3003'; // Update this for production

    // Get token metadata
    const [inputTokenMeta, outputTokenMeta] = await Promise.all([
      jupiterApi.lookupToken('SOL'),
      jupiterApi.lookupToken('USDT'),
    ]);

    if (!inputTokenMeta || !outputTokenMeta) {
      alert('Error fetching token metadata');
      return;
    }
